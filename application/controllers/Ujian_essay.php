...
class Ujian_essay extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('Essay_model', 'soal');
		$this->load->model('Ujian_essay_model', 'ujian_essay');
		$this->form_validation->set_error_delimiters('', '');

		$this->user = $this->ion_auth->user()->row();
		$this->mhs 	= $this->ujian_essay->getIdMahasiswa($this->user->username);
	}
	...
	public function akses_mahasiswa()
	{
		if (!$this->ion_auth->in_group('mahasiswa')) {
			show_error('Halaman ini khusus untuk mahasiswa mengikuti ujian_essay, <a href="' . base_url('dashboard') . '">Kembali ke menu awal</a>', 403, 'Akses Terlarang');
		}
	}
	...
	public function json($id = null)
	{
		...
		$this->output_json($this->ujian_essay->getDataEssay($id), false);
	}
	...
	public function master()
	{
		...
		$data = [
			...
			'judul'	=> 'Ujian',
			'subjudul' => 'Data Ujian',
			'dosen' => $this->ujian_essay->getIdPengajar($user->username),
		];
	...
	public function edit($id)
	{
		...
		$data = [
			...
			'skemalsp'	=> $this->soal->getskemalspPengajar($user->username),
			'dosen'		=> $this->ujian_essay->getIdPengajar($user->username),
			'ujian_essay'		=> $this->ujian_essay->getUjianById($id),
		];
	...
	public function validasi()
	{
		...
		$user 	= $this->ion_auth->user()->row();
		$dosen 	= $this->ujian_essay->getIdPengajar($user->username);
		$jml 	= $this->ujian_essay->getJumlahSoal($dosen->id_pengajar)->jml_soal;
		$jml_a 	= $jml + 1; // Jika tidak mengerti, silahkan baca user_guide codeigniter tentang form_validation pada bagian less_than

		$this->form_validation->set_rules('nama_essay', 'Nama Ujian', 'required|alpha_numeric_spaces|max_length[50]');
	...
	public function save()
	{
		...
		$nama_essay 	= $this->input->post('nama_essay', true);
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if ($method === 'add') {
				...
				$input['token']			= $token;
				$action = $this->master->create('m_essay', $input);
			} else if ($method === 'edit') {
				$id_essay = $this->input->post('id_essay', true);
				$action = $this->master->update('m_essay', $input, 'id_essay', $id_essay);
			}
	...
	public function delete()
	{
		...
		if (!$chk) {
			...
			if ($this->master->delete('m_essay', $chk, 'id_essay')) {
	...
	public function refresh_token($id)
	{
		...
		$refresh = $this->master->update('m_essay', $data, 'id_essay', $id);
	...
	public function list_json()
	{
		...
		$list = $this->ujian_essay->getListEssay($this->mhs->id_mahasiswa, $this->mhs->skema_id);
	...
	public function list()
	{
		...
		$data = [
			...
			'judul'		=> 'Ujian',
			'subjudul'	=> 'List Ujian',
			'mhs' 		=> $this->ujian_essay->getIdMahasiswa($user->username),
		];
		$this->load->view('_templates/dashboard/_header.php', $data);
	...
	{
		$id = $this->input->post('id_essay', true);
		$token = $this->input->post('token', true);
		$cek = $this->ujian_essay->getUjianById($id);
	...
	public function index()
	{
		...
		$ujian_essay 		= $this->ujian_essay->getUjianById($id);
		$soal 		= $this->ujian_essay->getSoal($id);

		$mhs		= $this->mhs;
		$h_essay 	= $this->ujian_essay->HslUjian($id, $mhs->id_mahasiswa);

		$cek_sudah_ikut = $h_essay->num_rows();
		...
		if ($cek_sudah_ikut < 1) {
			...
			$waktu_selesai 	= date('Y-m-d H:i:s', strtotime("+{$ujian_essay->waktu} minute"));
			...
			$this->master->create('h_essay', $input);
		...
		for ($i = 0; $i < sizeof($urut_soal); $i++) {
			...
			$ambil_soal 	= $this->ujian_essay->ambilSoal($pc_urut_soal1, $pc_urut_soal[0]);
		...
		if (!empty($soal_urut_ok)) {
			foreach ($soal_urut_ok as $s) {
				...
				$html .= '<div class="funkyradio-success" onclick="ret	urn simpan_sementara();">
	...
	public function simpan_satu()
	{
		...
		for ($i = 1; $i < $input['jml_soal']; $i++) {
			...
			$list_jawaban	.= "" . $input[$_tidsoal] . ":" . $jawaban_ . ":" . $input[$_ragu] . "<br>" . ",";
		}
		...
		// Simpan jawaban
		$this->master->update('h_essay', $d_simpan, 'id', $id_tes);
	...
	public function simpan_akhir()
	{
		...
		// Get Jawaban
		$list_jawaban  = $this->ujian_essay->getJawaban($id_tes);
		$jawaban_cos = "";
		// Pecah Jawaban
		...
		foreach ($pc_jawaban as $jwb) {
			...
			$cs = $this->cosine->similarity($jawaban, $kunci_jawaban);
			$csn1 = $cs * 100;
			$csn2 = round($csn1);
			$nilai_cs += $csn2;
			$jawaban == $cek_jwb->jawaban ? $jumlah_benar++ : $jumlah_salah++;
			$jawaban_cos	.= "" . $pc_dt[0] . ":" . number_format($cs, 3) . "" . "<br>";
		}

		$nilai 			= $nilai_cs / $jumlah_soal;

		$d_update = [
			'jml_benar'		=> $jumlah_benar,
			'nilai'			=> round($nilai),
			'nilai_bobot'	=> $jawaban_cos,
			'status'		=> 'N'
		];

		$this->master->update('h_essay', $d_update, 'id', $id_tes);

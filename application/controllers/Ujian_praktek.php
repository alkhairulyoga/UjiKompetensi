...
class Ujian_praktek extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('Praktek_model', 'soal');
		$this->load->model('Ujian_praktek_model', 'ujian_praktek');
		$this->form_validation->set_error_delimiters('', '');

		$this->user = $this->ion_auth->user()->row();
		$this->mhs 	= $this->ujian_praktek->getIdMahasiswa($this->user->username);
	}
	...
	public function akses_mahasiswa()
	{
		if (!$this->ion_auth->in_group('mahasiswa')) {
			show_error('Halaman ini khusus untuk mahasiswa mengikuti ujian_praktek, <a href="' . base_url('dashboard') . '">Kembali ke menu awal</a>', 403, 'Akses Terlarang');
		}
	}
	...
	public function json($id = null)
	{
		...
		$this->output_json($this->ujian_praktek->getDataPraktek($id), false);
	}
	...
	public function master()
	{
		...
		$data = [
			...
			'judul'	=> 'Ujian',
			'subjudul' => 'Data Ujian',
			'dosen' => $this->ujian_praktek->getIdPengajar($user->username),
		];
		$this->load->view('_templates/dashboard/_header.php', $data);
		$this->load->view('ujian_praktek/data');
	...
	public function add()
	{
		...
		$this->load->view('_templates/dashboard/_header.php', $data);
		$this->load->view('ujian_praktek/add');
		$this->load->view('_templates/dashboard/_footer.php');
	}
	...
	public function edit($id)
	{
		...
		$data = [
			...
			'skemalsp'	=> $this->soal->getskemalspPengajar($user->username),
			'dosen'		=> $this->ujian_praktek->getIdPengajar($user->username),
			'ujianpraktek'		=> $this->ujian_praktek->getUjianById($id),
		];

		$this->load->view('_templates/dashboard/_header.php', $data);
		$this->load->view('ujian_praktek/edit');
		$this->load->view('_templates/dashboard/_footer.php');
	}
	...
	public function validasi()
	{
		...
		$user 	= $this->ion_auth->user()->row();
		$dosen 	= $this->ujian_praktek->getIdPengajar($user->username);
		$jml 	= $this->ujian_praktek->getJumlahSoal($dosen->id_pengajar)->jml_soal;
		$jml_a 	= $jml + 1; // Jika tidak mengerti, silahkan baca user_guide codeigniter tentang form_validation pada bagian less_than

		$this->form_validation->set_rules('nama_praktek', 'Nama Ujian', 'required|alpha_numeric_spaces|max_length[50]');
	...
	public function save()
	{
		...
		$nama_praktek 	= $this->input->post('nama_praktek', true);
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if ($method === 'add') {
				...
				$input['token']			= $token;
				$action = $this->master->create('m_praktek', $input);
			} else if ($method === 'edit') {
				$id_praktek = $this->input->post('id_praktek', true);
				$action = $this->master->update('m_praktek', $input, 'id_praktek', $id_praktek);
			}
	...
	public function delete()
	{
		...
		if (!$chk) {
			...
			if ($this->master->delete('m_praktek', $chk, 'id_praktek')) {
	...
	public function refresh_token($id)
	{
		...
		$refresh = $this->master->update('m_praktek', $data, 'id_praktek', $id);
	...
	public function list_json()
	{
		...
		$list = $this->ujian_praktek->getListPraktek($this->mhs->id_mahasiswa, $this->mhs->skema_id);
	...
	public function list()
	{
		...
		$data = [
			...
			'judul'		=> 'Ujian',
			'subjudul'	=> 'List Ujian',
			'mhs' 		=> $this->ujian_praktek->getIdMahasiswa($user->username),
		];
		$this->load->view('_templates/dashboard/_header.php', $data);
		$this->load->view('ujian_praktek/list');
		$this->load->view('_templates/dashboard/_footer.php');
	...
	public function token($id)
	{
		...
		$data = [
			...
			'mhs' 		=> $this->ujian_praktek->getIdMahasiswa($user->username),
			'ujian_praktek'		=> $this->ujian_praktek->getUjianById($id),
			'encrypted_id' => urlencode($this->encryption->encrypt($id))
		];
		$this->load->view('_templates/topnav/_header.php', $data);
		$this->load->view('ujian_praktek/token');
		$this->load->view('_templates/topnav/_footer.php');
	}

	public function cektoken()
	{
		$id = $this->input->post('id_praktek', true);
		$token = $this->input->post('token', true);
		$cek = $this->ujian_praktek->getUjianById($id);
	...
	public function index()
	{
		...
		$ujian_praktek 		= $this->ujian_praktek->getUjianById($id);
		$soal 		= $this->ujian_praktek->getSoal($id);

		$mhs		= $this->mhs;
		$h_praktek 	= $this->ujian_praktek->HslUjian($id, $mhs->id_mahasiswa);

		$cek_sudah_ikut = $h_praktek->num_rows();
		...
		if ($cek_sudah_ikut < 1) {
			...
			foreach ($soal as $s) {
				...
				$soal_per->id_praktek 		= $s->id_praktek;
			...
			$waktu_selesai 	= date('Y-m-d H:i:s', strtotime("+{$ujian_praktek->waktu} minute"));
			...
			$this->master->create('h_praktek', $input);

			// Setelah insert wajib refresh dulu
			redirect('ujian_praktek/?key=' . urlencode($key), 'location', 301);
		}

		$q_soal = $h_praktek->row();
		...
		for ($i = 0; $i < sizeof($urut_soal); $i++) {
			...
			$ambil_soal 	= $this->ujian_praktek->ambilSoal($pc_urut_soal1, $pc_urut_soal[0]);
		...
		if (!empty($soal_urut_ok)) {
			foreach ($soal_urut_ok as $s) {
				...
				<div class="w-25">' . tampil_media($path . $s->file) . '</div></div>' . $s->soal . '<div class="funkyradio">';
				$html .= '<div class="funkyradio-success" onclick="return simpan_sementara();">
						<input type="text"  rows="100" cols="50" class="form-control summernote"  id="opsi_' .   '_' . $s->id_soal . '" name="opsi_' . $no . '" value="' .  '" '  . '></div>';
				$html .= '</div></div>';
				$no++;
			}
		}
		...
		$this->load->view('_templates/topnav/_header.php', $data);
		$this->load->view('ujian_praktek/sheet');
		$this->load->view('_templates/topnav/_footer.php');
	}
	...
	public function simpan_satu()
	{
		...
		// Simpan jawaban
		$this->master->update('h_praktek', $d_simpan, 'id', $id_tes);
	...
	public function simpan_akhir()
	{
		...
		// Get Jawaban
		$list_jawaban = $this->ujian_praktek->getJawaban($id_tes);
		...
		$this->master->update('h_praktek', $d_update, 'id', $id_tes);
		$this->output_json(['status' => TRUE, 'data' => $d_update, 'id' => $id_tes]);
		$this->mhs 	= $this->ujian_praktek->getIdMahasiswa($this->user->username);



		$config['upload_path']        = realpath('./uploads/import/');
		...
		if (!$this->upload->do_upload('opsi_')) {

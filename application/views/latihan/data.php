			...
			<div class="form-group col-sm-4 text-center">
				<?php if ($this->ion_auth->is_admin()) : ?>
					<select id="skemalsp_filter" class="form-control select2" style="width:100% !important">
						<option value="all">Semua skemalsp</option>
				...
				<?php if ($this->ion_auth->in_group('dosen')) : ?>
					<input id="klsp_id" value="<?= $klsp->nama_klsp; ?>" type="text" readonly="readonly" class="form-control">
				<?php endif; ?>
			</div>
			<div class="col-sm-4">
				<div class="pull-right">
					<a href="<?= base_url('latihan/add') ?>" class="btn bg-purple btn-flat btn-sm"><i class="fa fa-plus"></i> Buat latihan</a>
					<button type="button" onclick="reload_ajax()" class="btn btn-flat btn-sm bg-maroon"><i class="fa fa-refresh"></i> Reload</button>
				</div>
			</div>
		</div>
	</div>
	<?= form_open('latihan/delete', array('id' => 'bulk')) ?>
	<div class="table-responsive px-4 pb-3" style="border: 0">
		<table id="latihan" class="w-100 table table-striped table-bordered table-hover">
			<thead>
				<tr>
...
<script src="<?= base_url() ?>assets/dist/js/app/latihan/data.js"></script>

$(document).ready(function () {
    ...
    $('#formlatihanmodul input, #formlatihanmodul select').on('change', function () {
        $(this).closest('.form-group').eq(0).removeClass('has-error');
        $(this).nextAll('.help-block').eq(0).text('');
    });

    $('#formlatihanmodul').on('submit', function (e) {
        e.preventDefault();
            ...
            success: function (data) {
                ...
                if (data.status) {
                    Swal({
                        ...
                        window.location.href = base_url+"latihanmodul/master";
                    });
                } else {
                    if (data.errors) {

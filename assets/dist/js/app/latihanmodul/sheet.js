...
function simpan() {
    simpan_sementara();
    var form = $("#latihanmodul");

    $.ajax({
        type: "POST",
        url: base_url + "latihanmodul/simpan_satu",
        data: form.serialize(),
        dataType: 'json',
...
function selesai() {
    ...
    $.ajax({
        type: "POST",
        url: base_url + "latihanmodul/simpan_akhir",
        data: { id: id_tes },
        beforeSend: function () {
            simpan();
            // $('.ajax-loading').show();    
        ...
        success: function (r) {
            console.log(r);
            if (r.status) {
                window.location.href = base_url + 'latihanmodul/list';
            }
        }
    });
}
...
function waktuHabis() {
    selesai();
    alert('Waktu latihanmodul telah habis!');
}

$(document).ready(function () {
    ...
    $('#formujian_praktek input, #formujian_praktek select').on('change', function () {
        $(this).closest('.form-group').eq(0).removeClass('has-error');
        $(this).nextAll('.help-block').eq(0).text('');
    });

    $('#formujian_praktek').on('submit', function (e) {
        e.preventDefault();
            ...
            success: function (data) {
                ...
                if (data.status) {
                    Swal({
                        ...
                        window.location.href = base_url+"ujian_praktek/master";
                    });
                } else {
                    if (data.errors) {

...
function simpan() {
    simpan_sementara();
    var form = $("#ujian_praktek");

    $.ajax({
        type: "POST",
        url: base_url + "ujian_praktek/simpan_satu",
        data: form.serialize(),
        dataType: 'json',
...
function selesai() {
    ...
    $.ajax({
        type: "POST",
        url: base_url + "ujian_praktek/simpan_akhir",
        data: { id: id_tes },
        beforeSend: function () {
            simpan();
            // $('.ajax-loading').show();    
        ...
        success: function (r) {
            console.log(r);
            if (r.status) {
                window.location.href = base_url + 'ujian_praktek/list';
            }
        }
    });
}
...
function waktuHabis() {
    selesai();
    alert('Waktu ujian_praktek telah habis!');
}
